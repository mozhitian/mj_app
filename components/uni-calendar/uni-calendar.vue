<template>
	<view>
		...
		<view :style="{display:blo}">
			<wpicker :type="'date'"></wpicker>
		</view>
		<view v-if="blo=='none'">
			<text style="font-size: 14px;position: absolute;left: 55%;top: 2.5%;color: #D8D8D8;">请选择</text>
		</view>
	</view>
	</view>
</template>
...
<script>
	...
	import wpicker from "../w-picker-jcxm/index.vue"
	...
	export default {
		...
		created() {
			this.init();
			setTimeout(function() {
				Bus.$emit("postJccs", ['市政工程-路面检测', '弯沉', '全部检测']);
			}, 500);
			var _this=this;
			setTimeout(function(){
				_this.blo='block';
		...
		methods: {
			...
			isDisableBefore(year, month, date) {
				const nowDate = "2010-1-01" || new Date() //设定可以点击获取时间的初始
